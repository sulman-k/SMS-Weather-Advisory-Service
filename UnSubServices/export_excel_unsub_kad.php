...
if (isset($userid) && in_array($role, $cro)) {
    ...
    doLog('[export_excel_unsub_kad][NEW][start date: ' . $st . ' ][NEW][end date: ' . $end . ' ] [NEW][cellno: ' . $cellno . ' ][is date : '. $is_dt . ']');

    $sql = "SELECT cellno,sub_dt,unsub_dt,sub_mode,unsub_mode,charge_attempt_dt,last_charge_dt,next_charge_dt,grace_expire_dt FROM subscriber_unsub $date_check";

    doLog('[export_excel_unsub_kad][NEW][sql: ' . $sql . ' ]');
    ...
    $objSheet->setTitle('K-Amdani UNSubscriber Report');
    $rowCount = 1;
    $objSheet->getColumnDimension('A')->setAutoSize(true);

    doLog('[export_excel_unsub_kad][NEW][sql: Excel obj is created ]');
    ...
    try {
        ...
        $objSheet->setCellValue('A1', 'K-Amdani unSubscriber Report');
        ...
        doLog('[export_excel_unsub_kad][NEW][Query is executed ]');
        ...
        doLog('[export_excel_unsub_kad][NEW][Excel2007]');

        $fileName = 'K-Amdani unsubscriber_report';
        ...
        doLog('[export_excel_unsub_kad][Exception][' . $e . ']');
    }
