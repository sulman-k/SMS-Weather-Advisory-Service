    ...
    if (isset($state) && isset($date) && isset($location_id) && isset($min_temp) && isset($max_temp) && isset($condition_id) && isset($condition_3day_id)) {
        ...
        if ($max_temp > $min_temp) {
            ...
            if (!strcmp($state, 'gb')) {
                ...
                doLog('[sr_saveAWdata][GB][Find Location] [location_id ' . $location_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $row = $result->fetch_assoc();
                    ...
                    doLog('[sr_saveAWdata][GB][Find Condition] [condition_id ' . $condition_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        $row = $result->fetch_assoc();
                        ...
                        doLog('[sr_saveAWdata][GB][Find Condition] [Evening condition_id ' . $eve_condition_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            ...
                            doLog('[sr_saveAWdata][GB][Find Future Condition] [condition_3day_id ' . $condition_3day_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                $row = $result->fetch_assoc();
                                ...
                                doLog('[sr_saveAWdata][GB][Find Future Condition] [Evening condition_3day_id ' . $eve_condition_3day_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    $row = $result->fetch_assoc();
...
//                                    values ('','$date',$location_id,$min_temp,$max_temp,$condition_id,$condition_3day_id,'$weather_file_name','$weather_3day_file_name','$sms_text','$hex_sms_text',$eve_condition_id,$eve_condition_3day_id,$eve_min_temp,$eve_max_temp,'$eve_weather_file_name','$eve_weather_3day_file_name','$eve_sms_text','$eve_hex_sms_text')";
                                        ...
                                        $sql = "INSERT INTO `aw_daily_weather` (`id`, `date`, `state`, `district`, `min_temp`, `max_temp`, `now_file_id`, `now_file_name`, `future_file_id`, `future_file_name`, `sms_text` , `hex_sms_text` , `aw_type`, `upload_dt`, `status`) VALUES (null, '$date', '$state', '$location_id', '$min_temp', '$max_temp', $condition_id , '$weather_file_name', $condition_3day_id, '$weather_3day_file_name', '$sms_text' , '$hex_sms_text' , $aw_type, NOW(), 100)";
                                        doLog('[sr_saveAWdata][GB][Create SMS String] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                                        $result = $conn->query($sql);
                                        $last_id = mysqli_insert_id($conn);

                                        $sql = "INSERT INTO `aw_daily_weather` (`id`, `date`, `state`, `district`, `min_temp`, `max_temp`, `now_file_id`, `now_file_name`, `future_file_id`, `future_file_name`, `sms_text` , `hex_sms_text` ,`aw_type`, `upload_dt`, `status`) VALUES (null, '$date', '$state', '$location_id', '$eve_min_temp', '$eve_max_temp', $eve_condition_id , '$eve_weather_file_name', $eve_condition_3day_id, '$eve_weather_3day_file_name', '$eve_sms_text' , '$eve_hex_sms_text', $aw_type_eve, NOW(), 100)";
                                        doLog('[sr_saveAWdata][GB][Create SMS String] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                                        $result = $conn->query($sql);

                                        $last_id_eve = mysqli_insert_id($conn);
                                        $response['success'] = 100;
                                        $response["msg"] = "SMS string has been successfully created";
                                        $response["last_id"] = $last_id;
                                        //$response["last_id_eve"] = $last_id_eve;
                                        $response["src"] = $state;
                ...
                $sql = "select * from district_lang_def where district = '$location_id'";
                doLog('[sr_saveAWdata]['. $state .'][Find Location] [location_id ' . $location_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $row = $result->fetch_assoc();
                    $loc_sms_name = $row['aw_sms_txt'];
                    $loc_file_name = $row['aw_file_name'];
                    $sql = "select * from aw_conditions where id=$condition_id";
                    doLog('[sr_saveAWdata]['. $state .'][Find Condition] [condition_id ' . $condition_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        $row = $result->fetch_assoc();
                        $weather_sms_name = $row['sms_txt'];
                        ...
                        $sql = "select * from aw_conditions where id=$condition_3day_id";
                        doLog('[sr_saveAWdata]['. $state .'][Find Future Condition] [condition_3day_id ' . $condition_3day_id . '] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            $weather_3day_sms_name = $row['sms_txt'];
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                $sql = "INSERT INTO `aw_daily_weather` (`id`, `date`, `state`, `district`, `min_temp`, `max_temp`, `now_file_id`, `now_file_name`, `future_file_id`, `future_file_name`, `sms_text` , `hex_sms_text` , `aw_type`, `upload_dt`, `status`)
                                        VALUES (null, '$date', '$state', '$location_id', '$min_temp', '$max_temp', $condition_id , '$weather_file_name', $condition_3day_id, '$weather_3day_file_name', '$sms_text' , '$hex_sms_text' ,$aw_type, NOW(), 100)";
                                doLog('[sr_saveAWdata]['.$state.'][Create SMS String] [user_id: ' . $userid . '][sql : ' . $sql . ']');
                                $result = $conn->query($sql);
                                $last_id = mysqli_insert_id($conn);
                                $response['success'] = 100;
                                $response["msg"] = "SMS string has been successfully created";
