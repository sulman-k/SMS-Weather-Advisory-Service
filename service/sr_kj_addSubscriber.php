...
if (isset($userid) && in_array($role, $cro) ||  in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) == 11) {
        ...
        $url = KJ_SUBSCRIBE_URL;
        $fields = array(
            'cellno' => urlencode($cellno),
            "subMode" => DEFAULT_SUB_MODE,
            "serviceId" => urlencode($serviceId)
        );
        ...
        doLog("URL for watson Jeewan Subscrition  - " . $url . $fields_string);
        //open connection
        $ch = curl_init();
        ...
        $response["issub"] = $result->IS_SUBSCRIBED;
        doLog("[sr_kj_addSubscriber][Response Code] - " . $result->responseCode . " - [API Response Message] -" . $result->message . " - IS_SUB - " . $result->IS_SUBSCRIBED);
        //now calling charging API
        if ($result->IS_SUBSCRIBED === 'Y') {
...
//             $fields_string = substr_replace($fields_string, "", -1);
//
            doLog("URL for watson Jeewan Charging  - " . $url . 'cellno=' . urlencode($cellno));
//             //open connection
            $ch = curl_init();
            ...
            doLog('[sr_kj_addSubscribers][] [sql: ' . $sql . ']');
            ...
            doLog('[sr_kj_addSubscriber] [cellno: ' . $cellno . '] is not subscribed');
        }
    } else {
            $response["msg"] = "Cellno is not valid";
            doLog('[sr_kj_addSubscriber][Checking in Subscriber] [cellno: ' . $cellno . '] is not valid');
    }
