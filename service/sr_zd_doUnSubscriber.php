...
if (isset($userid) && in_array($role, $cro) ||  in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) <= 11) {
        ...
        doLog("URL for Zarai Directory UnSub  - " . $url . $fields_string);
        ...
        doLog("[sr_zd_dounSubscribe][Response Code] - ".$result->responseCode ." - [API Response Message] -".$result->message);
      ...
      /*  if(in_array($role,$paid_wall_unsub) && $result->responseCode===100){
            ...
            doLog('[sr_zd_doUnsubscribe][Update subscriber] [cellno: ' . $cellno . '][Query]'.$sql);
            ...
            if($result){
                ...
                doLog('[sr_zd_doUnsubscribe][Update subscriber_unsub_history] [cellno: ' . $cellno . '][Query]'.$sql);
                $result=$zd_conn->query($sql);
                if($result){
                    $response['success']=100;
        ...
        doLog('[sr_zd_doUnsubscribe][Checking in Subscriber] [cellno: ' . $cellno . '] is not valid');
    }
