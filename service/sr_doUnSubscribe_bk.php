...
if (isset($userid) && in_array($role, $cro)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) <= 11) {
        ...
        doLog('[sr_doUnsubscribe][Checking in Subscriber] [sql: ' . $sql . ']');
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            ...
            $result = $conn->query($sql);
            doLog('[sr_doUnsubscribe][Checking in Subscriber Unsub] [sql: ' . $sql . ']');
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
...
//                    $mt_sms = $row_sub_stats['mt_sms'];
                    ...
                    try {
                        ...
                        $result = '';
                        $sql = "INSERT INTO `subscriber_unsub_history` (`id`, `cellno`, `location`, `state`, `srvc_id`, `lang`, `sub_name`, `first_sub_dt`, `last_sub_dt`, `last_unsub_dt`, `sub_mode`, `bc_mode`, `alert_type`,  `last_charge_dt`, `status`, `last_call_dt`, `last_update_dt`, `last_update_by`, `mo_call` , `mt_call`, `mo_sms`, `mt_sms`, `gender`, `occupation`, `village`, `land_size`, `land_unit`, `comment`, `dt`, `unsub_mode`,`has_consented`) SELECT NULL, `cellno`, `location`, `state`, `srvc_id`, `lang`, `sub_name`, `first_sub_dt`, `last_sub_dt`, `last_unsub_dt`, `sub_mode`, `bc_mode`, `alert_type`, `last_charge_dt`, `status`, `last_call_dt`, `last_update_dt`, `last_update_by`, `mo_call` , `mt_call`, `mo_sms`, `mt_sms`, `gender`, `occupation`, `village`, `land_size`, `land_unit`, `comment`, `dt`, `unsub_mode` ,`has_consented` FROM subscriber_unsub WHERE cellno = '$cellno'";
                        doLog('[sr_doUnsubscribe][] [INSERT: subscriber_unsub_history ' . $userid . '][sql: ' . $sql . ' ]');
                        $result = $conn->query($sql);
                        if ($result === false) {
                        ...
                        doLog('[sr_doUnsubscribe][] [DELETE: subscriber ' . $userid . '][sql: ' . $sql . ' ]');
                        $result = $conn->query($sql);
                        if ($result === false) {
                            throw new Exception('SQL Error: ' . $conn->error);
                        }
                        ...
                        doLog('[sr_doUnsubscribe][] [DELETE: sub_profile ' . $userid . '][sql: ' . $sql . ' ]');
                        $result = $conn->query($sql);
                        if ($result === false) {
                            throw new Exception('SQL Error: ' . $conn->error);
                        }
                        ...
                        doLog('[sr_doUnsubscribe][] [DELETE: sub_stats ' . $userid . '][sql: ' . $sql . ' ]');
                        $result = $conn->query($sql);
                        if ($result === false) {
                            throw new Exception('SQL Error: ' . $conn->error);
                        }
                    ...
                    doLog('[sr_doUnsubscribe][Checking in Subscriber] [sql: ' . $response["msg"] . ']');
                }
        ...
        doLog('[sr_doUnsubscribe][Checking in Subscriber] [cellno: ' . $cellno . '] is not valid');
    }
