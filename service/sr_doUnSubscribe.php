...
if (isset($userid) && in_array($role, $cro) || in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) <= 11) {
        ...
        doLog("[sr_dounSubscribe][Response Code] - ".$result->responseCode ." - [API Response Message] -".$result->message);
        ...
        if(in_array($role,$paid_wall_unsub) && $result->responseCode===100){
            ...
            doLog('[sr_doUnsubscribe][Update subscriber] [cellno: ' . $cellno . '][Query]'.$sql);
            $result=$conn->query($sql);
            if($result){
              $sql="UPDATE subscriber_unsub_history SET unsub_mode  = '$unsub_mode' WHERE cellno = '$cellno' ORDER BY last_unsub_dt DESC LIMIT 1";
              doLog('[sr_doUnsubscribe][Update subscriber_unsub_history] [cellno: ' . $cellno . '][Query]'.$sql);
              $result=$conn->query($sql);
              if($result){
                  $response['success']=100;
