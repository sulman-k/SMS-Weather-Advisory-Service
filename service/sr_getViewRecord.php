...
if (isset($userid) && in_array($role, $admin_content)) {
    ...
    $sql = "SELECT * FROM aw_daily_weather WHERE district = '$loc_id' AND `date` = '$date' AND (`aw_type` = 100 OR `aw_type` = 200 OR `aw_type` = 300 OR `aw_type` = 400) ORDER BY `date` DESC LIMIT 1";
    $result = $conn->query($sql);
    doLog('[sr_getViewRecord][auto_weather] [sql: ' . $sql . ']');
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        ...
        if (!strcmp($state, 'gb')) {
            ...
            $result = $conn->query($sql);
            doLog('[sr_getViewRecord][Location] [sql: ' . $sql . ']');
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                $max_file = '';
                $min_file = '';
...
//                    }
                ...
                if ($result->num_rows > 0) {
                    ...
                    $result = $conn->query($sql);
                    doLog('[sr_getViewRecord][condition] [sql: ' . $sql . ']');
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        $row = $result->fetch_assoc();
                        ...
                        $result = $conn->query($sql);
                        doLog('[sr_getViewRecord][future condition] [sql: ' . $sql . ']');
                ...
                $result = $conn->query($sql);
                doLog('[sr_getViewRecord][Evening][auto_weather] [sql: ' . $sql . ']');
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $row = $result->fetch_assoc();
...
//                        }
                ...
                if ($result->num_rows > 0) {
                    ...
                    $result = $conn->query($sql);
                    doLog('[sr_getViewRecord][eve condition] [sql: ' . $sql . ']');
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        $row = $result->fetch_assoc();
...
//                            $response["files_3"] = $files_3;
            ...
            $response["msg"] = "No record found against state";
        }
    }else {
        $response["msg"] = "No record found against these parameters";
    }
