...
if (isset($userid) && in_array($role, $cro)||  in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) == 11) {
        ...
        doLog("URL for Subscrition  - " .$url.$fields_string);
        //open connection
        $ch = curl_init();
...
/*
            $url = KZ_CHARGE_URL;
            doLog("URL for KZ Charging  - " . $url . 'cellno=' .urlencode($cellno));
//             //open connection
            $ch = curl_init();
        ...
        if($result->IS_SUB != "Y")
        {
            $sql = "SELECT s.*   FROM subscriber s  WHERE `status` > 0 AND `state`='$result->state' AND cellno = '$cellno'";
