...
if (isset($userid) && in_array($role, $cro) ||  in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) == 11) {
        ...
        doLog("URL for watson rose Subscrition  - " . $url . $fields_string);
        //open connection
        $ch = curl_init();
...
//             $fields_string = substr_replace($fields_string, "", -1);
//
            doLog("URL for watson rose Charging  - " . $url . 'cellno=' .urlencode($cellno));
//             //open connection
            $ch = curl_init();
        ...
        if ($result->IS_SUB = "Y") {
            ...
            doLog('[sr_kam_addSubscribers][] [sql: ' . $sql . ']');
            ...
            doLog("[sr_kam_addSubscriber][Response Code] - " . $result->responseCode . " - [API Response Message] -" . $result->message . " - IS_SUB - " . $result->IS_SUB);
        } else {
            $response["msg"] = "Cellno is not valid";
            doLog('[sr_kam_addSubscriber][Checking in Subscriber] [cellno: ' . $cellno . '] is not valid');
        }
