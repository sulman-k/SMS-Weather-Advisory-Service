...
if (isset($state) && isset($userid) && in_array($role, $cro) || in_array($role,$paid_wall_unsub)) {
    ...
    doLog('[sr_ka_loadSubscribers][] [sql_total: ' . $sql_total . ']');
    $result = $ka_conn->query($sql_total);
    $row = $result->fetch_assoc();
    $total_count  = $row['total'];

    $sql = "SELECT * FROM subscriber  WHERE `status` > 0  $date_check  order by first_sub_dt desc LIMIT $offset ,$page_limit ";
    doLog('[sr_ka_loadSubscribers][] [sql: ' . $sql . ']');
