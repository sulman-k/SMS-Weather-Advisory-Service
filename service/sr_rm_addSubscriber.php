...
if (isset($userid) && in_array($role, $cro) ||  in_array($role,$paid_wall_unsub)) {
    ...
    if (strlen($cellno) > 0 && strlen($cellno) == 11) {
        ...
        doLog("URL for Radio Magan Subscrition  - " . $url . $fields_string);
        //open connection
        $ch = curl_init();
...
//             $fields_string = substr_replace($fields_string, "", -1);
//
            doLog("URL for Radio Magaan Charging  - " . $url . 'cellno=' .urlencode($cellno));
//             //open connection
            $ch = curl_init();
        ...
        if ($result->IS_SUB = "Y") {
            ...
            doLog('[sr_rm_addSubscribers][] [sql: ' . $sql . ']');
            ...
            doLog("[sr_rm_addSubscriber.php][Response Code] - " . $result->responseCode . " - [API Response Message] -" . $result->message . " - IS_SUB - " . $result->IS_SUB);
        } else {
            $response["msg"] = "Cellno is not valid";
            doLog('[sr_rm_addSubscriber.php][Checking in Subscriber] [cellno: ' . $cellno . '] is not valid');
        }
